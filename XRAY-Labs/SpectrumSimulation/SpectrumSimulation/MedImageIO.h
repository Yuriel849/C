...
private:
	...
	static void writeUS(uint16_t group, uint16_t element, uint16_t value, std::ofstream &out);
	static void writeDS(uint16_t group, uint16_t element, double value, std::ofstream &out);
	static void writeCS(uint16_t group, uint16_t element, const char *text, std::ofstream &out);    // not tested
	...
	// elementary methods for CSV-files
	static void writeCsv(const cMedImage<T> &image, std::ofstream &out);
	static void readCsv(cMedImage<T> &image, std::ifstream &inp);
...
public:
	...
	// interfaces for CSV-files
	static void writeCsv(const cMedImage<T> &image, const std::string &filename);
	static void writeCsv(const cMedImage<T> &image, const std::wstring &filename);
	static void readCsv(cMedImage<T> &image, const std::string &filename);
	static void readCsv(cMedImage<T> &image, const std::wstring &filename);
};
...
template<class T> void cMedImageIO<T>::cMedImageIO::writeUS(
	uint16_t group,	// [in] group if tag
	uint16_t element,	// [in] element of tag
	uint16_t value,	// [in] value of tag
...
//=============================================================================
template<class T> void cMedImageIO<T>::cMedImageIO::writeCS(
	uint16_t group,	// [in] group if tag
	uint16_t element,	// [in] element of tag
	const char *text,			// [in] value of tag
	std::ofstream &out)		// [inout] output stream
{
	size_t size;	// size of tag
	char str[17];	// number as string
	...
	std::ofstream &out)			// [inout] stream to be written into
{
	...
	try {
		...
		infoHeader.infoSize = 40;			// size of info header
		infoHeader.width = image.nCol;		// width of image in pixels
		infoHeader.height = image.nRow;		// heigt of image in pixels
		infoHeader.height *= -1;			// flip image vertically
		infoHeader.planes = 1;				// number of image layers (always 1)
		infoHeader.bitCount = 8;			// bits per pixel (here 8 bit to access LUT)
		infoHeader.compression = 0;			// type of compression (0 for no compression)
		infoHeader.imageSize = 0;			// size of pixel data (0 = auto detect)
		infoHeader.xPixelsPerMeter = 7000;	// pixel per meter in x-direction (here 1 pixel = 143 µm)
		infoHeader.yPixelsPerMeter = 7000;	// pixel per meter in y-direction (here 1 pixel = 143 µm)
		infoHeader.colorUsed = 256;			// no. of colours in LUT
		infoHeader.colorImportant = 256;	// improtant colours in LUT
		...
		for (r = 0; r < image.nRow; r++) {
			for (c = 0; c < image.nCol; c++) {
				...
				else byte = (BYTE)(255.999 * (dbl - pvBlack) / (pvWhite - pvBlack));
...
//=============================================================================
template<class T> void cMedImageIO<T>::cMedImageIO::writeCsv(
	const cMedImage<T> &image,		// [in] image to be written in file
	std::ofstream &out)			// [inout] stream to write image to
{
	// check if file open
	if(!out.is_open())
	...
	// write image size
	out << image.nRow << "," << image.nCol << std::endl;
...
//=============================================================================
template<class T> void cMedImageIO<T>::cMedImageIO::readCsv(
	cMedImage<T> &image,		// [in] image to be written in file
	std::ifstream &inp)			// [inout] stream to write image to
{
	unsigned rows, cols;	// image size
	char ch;
	...
	inp >> ch;
	if(ch!=',' && ch!=';')
		throw std::runtime_error("Unexpected character in CSV-file.");
	inp >> cols;
	if(rows<=0 || cols<=0) throw std::runtime_error("Invalid image size in CSV-file.");

	// read pixel data
	image.create(rows, cols);
	for(unsigned r=0; r<image.nRow; r++) {
		inp >> image[r][0];
		for(unsigned c=1; c<image.nCol; c++) {
			inp >> ch;
			if(ch!=',' && ch!=';')
				throw std::runtime_error("Unexpected character in CSV-file.");
			inp >> image[r][c];
	...
	if(inp.eof()) throw std::runtime_error("Unexpected end of file in CSV-file.");
}
