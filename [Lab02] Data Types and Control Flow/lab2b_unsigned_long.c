/* Author	   : Myungjun Kim
   Contents    : Version of "lab2b" using UNSIGNED LONGS.
...
int lab2b_unsigned_long_main()
{
	...
	/*
		Data type "unsigned long (int)" for numbering, onField, sum
			--> from numbering == 33, onField shows 0 and sum shows 4294967295d int and overflow occurs
				onField is always 0 because the figure is always larger than the maximum value of unsigne
				sum is always 4294967295 because onField always adds 0 to the value of sum
			--> this machine uses 32-bits for the unsigned long data type
			--> this machine uses the same size of memory for unsigned int & unsigned long data types
	 */

	// Variable for sequential numbering of chessboard fields (range 1 ~ 64)
	unsigned long numbering = 0;
	// Variable for storing the "On Field" value
	unsigned long onField = 0;
	...
	while (numbering < 64) {
		onField = (unsigned long)pow(2, numbering); // Calculate the "On Field" value; numbering starts from 0 to avoid arithmetic within pow()
		sum += onField; // Add the "On Field" value to the sum

		printf("%5lu | %8lu | %7lu (= %.1e)\n", ++numbering, onField, sum, (double)sum); // %lu is used for unsigned longs
